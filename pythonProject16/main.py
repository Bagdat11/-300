...
# Render үшін қатесіз абсолютті жол
...
def attend_submit(
    ...
    st = (
        supabase.table("student")
        .select("studentid, device_id, fullname, parent_telegram_id")
        .eq("studentid", sid)
        .limit(1)
        .execute()
        .data
    ...
    student_data = st[0]
    db_device = student_data.get("device_id")
    ...
    # Ата-анаға Telegram арқылы хабарлама жіберу
    parent_tg = student_data.get("parent_telegram_id")
    if parent_tg and TELEGRAM_BOT_TOKEN:
        try:
            requests.post(
                f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage",
                data={
                    "chat_id": parent_tg,
                    "text": f"Хабарландыру: Балаңыз/қызыңыз {student_data.get('fullname')} бүгін сабаққа келді ({now.strftime('%H:%M:%S')})."
                },
                timeout=5
            )
        except Exception:
...
async def bot_webhook(request: Request):
    try:
        ...
        if "message" in data and TELEGRAM_BOT_TOKEN:
            ...
            if text.startswith("/register "):
                ...
                # Студентті базадан толық аты-жөні бойынша іздеу
                res = supabase.table("student").select("studentid, fullname").ilike("fullname", f"%{student_name}%").limit(1).execute()
                
                if res.data:
                    st_record = res.data[0]
                    supabase.table("student").update({"parent_telegram_id": str(chat_id)}).eq("studentid", st_record["studentid"]).execute()
                    msg = f"Сәтті тіркелдіңіз! Оқушы: {st_record['fullname']}"
                else:
                    msg = "Мұндай аты бар студент табылмады. Дұрыс жазғаныңызды тексеріңіз."
